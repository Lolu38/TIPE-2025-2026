...
def add_to_steps(nbr_steps, addition):
    ...
    try:
        ...
        for ep in range(N_EPISODES):
            ...
            for step in range(int(nbr_steps)):
                ...
                # --- centrage sur la piste ---
                cx, cy = obs_proc.shape[1]//2, obs_proc.shape[0]//2
                road_check = sum(is_road_pixel(obs_proc[cy, cx+dx]) for dx in [-2, -1, 0, 1, 2])
                centering_bonus = 0.05 if road_check >= 4 else 0.0
                step_reward += centering_bonus
            ...
            if ep % 5 == 0:
                ...
                save_q_table(Q, "Save_folder/Merged_table/tot_q_table.csv")
